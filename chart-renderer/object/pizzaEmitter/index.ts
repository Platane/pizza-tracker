...
import { createPizzaTileSet } from "@pizza-tracker/pizza-drawer/createPizzaTileSet";
...
export const create = (gl: WebGLRenderingContext) => {
  ...
  // init sprite
  const texture = createPizzaTileSet(256 / n, n);
  ...
  return {
    ...
    update: () => {
      ...
      while (t * emitRate > p) {
        ...
        if (source) world.spawn(source, v0, v0Noise);
