...
const getYat = (points: Array<{ x: number; y: number }>, x: number) => {
  let ia = 0;
  for (; points[ia] && points[ia].x < x; ia++);

  if (!points[ia]) return points[ia - 1].y;

  if (ia == 0) return points[0].y;

  if (points[ia].x == points[ia - 1].x) return points[ia].y;

  const k = (x - points[ia - 1].x) / (points[ia].x - points[ia - 1].x);

  return k * points[ia].y + (1 - k) * points[ia - 1].y;
};
...
const createMatrixBuilder = () => {
  ...
  return {
    ...
    build: (k: number, tx: number, ty: number) => {
      ...
      const rho =
        -Math.PI / 2 + tx * Math.PI * 0.3 + ((Math.PI * (6 - k)) / 12) * 0.23;
      const phy =
        Math.PI * 0.065 - ty * Math.PI * 0.2 + ((Math.PI * k) / 12) * 0.12;
      const r = 4 + k / 12;
      ...
      mat4.multiply(
        m,
        m,
        mat4.fromTranslation(mat4.create(), [0, (-k / 12) * 1.5, -r])
      );
...
export const createChartRenderer = (canvas: HTMLCanvasElement) => {
  // prettier-ignore
  const gl =
    canvas.getContext("webgl2", WEBGL_OPTIONS) as WebGLRenderingContext ||
    canvas.getContext("webgl-experimental2", WEBGL_OPTIONS ) as WebGLRenderingContext ||
    canvas.getContext("webgl", WEBGL_OPTIONS) as WebGLRenderingContext ||
    canvas.getContext("webgl-experimental", WEBGL_OPTIONS) as WebGLRenderingContext;
  ...
  const render = () => {
    ...
    [
      ...
      // renderers.cube,
    ].forEach(({ draw }) => draw(worldMatrix.get()));
  };
  ...
  const loop = () => {
    ...
    renderers.pizzaEmitter.setV0(
      vec3.set(v0, u * 1.7, u * 1, 0),
      vec3.set(v0Noise, 0.8 + u * 5, 0.5 + u * 3.7, 1.2 + u * 3)
    );
  ...
  return {
    ...
    setK: (u: number) => {
      ...
      renderers.pizzaEmitter.setSources(
        lines.map(({ points }, i) => vec3.fromValues(k, getYat(points, k), i))
      );
    },
